...
class SMPLModel(Module):
  def __init__(self, device=None, model_path='E:/3D_HUMAN/Code/SMPL-master/model_processed/model_f.pkl'):
    ...
    self.J_regressor = torch.from_numpy(params["J_regressor"].todense()).type(torch.float32).to(self.device)
    self.weights = torch.from_numpy(params["weights"]).type(torch.float32).to(self.device)
    self.posedirs_SMPL = torch.from_numpy(params["posedirs"]).type(torch.float32).to(self.device)
  ...
  def forward(self, T_hat, pose, trans):
    ...
    J = torch.matmul(self.J_regressor, T_hat.repeat(batch_num, 1, 1))
    ...
    pose_offset = torch.tensordot(lrotmin, self.posedirs_SMPL, dims=([1], [2]))
    v_posed = T_hat.repeat(batch_num, 1, 1) + pose_offset

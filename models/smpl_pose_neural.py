...
class SMPLModel(Module):
  def __init__(self, device=None, model_path='E:/3D_HUMAN/Code/SMPL-master/model_processed/model_f.pkl', disable_posedirs = True):
    ...
    print("Disable posedirs ? ", self.disable_posedirs)
  ...
  def forward(self, T_hat, J_hat, weights, posedirs, pose, trans):
    ...
    """
          ...
          Args:
              ...
              posedirs: a neural network, transforms R (207,) into pose blend shapes offset.
    ...
    # weights = leaky_relu(weights, 0.01)
    # weights = weights / (torch.norm(weights, dim=1, keepdim=True, p = 1))   
    ...
    if self.disable_posedirs == True:
      # print("Disabled posedirs")
      pose_offset = torch.tensor([0.], dtype=torch.float32).to(self.device)
      v_posed = T_hat.repeat(batch_num, 1, 1)

    else: 
      # print("Activated posedirs")
      R_cube = R_cube_big[:, 1:, :, :]
      I_cube = (torch.eye(3, dtype=torch.float32).unsqueeze(dim=0) + \
      ...
      pose_offset = posedirs(lrotmin)
      v_posed = T_hat.repeat(batch_num, 1, 1) + pose_offset.reshape(-1, 6890, 3)
    # posedirs_neural_MI version
    # pose_offset = posedirs(verts=T_hat.reshape((20670,)).repeat(batch_num, 1), joints=lrotmin)
